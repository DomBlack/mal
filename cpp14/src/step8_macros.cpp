...
namespace mal {
  ...
  DEF_SPECIALS(defmacro, "defmacro!", {
    ARG_COUNT(2);
    ARG_CAST(symbol, 1, Symbol);
    ARG_EVAL(value,  2, env);
    CAST_NODE(func, value, UserFunc);

    auto macro = func->asMacro();

    // Set the enviroment variable
    env->set(symbol->value, macro);
  ...
  /// Checks if the given AST is a macro call
  bool isMacroCall(ast::TypePtr ast, runtime::EnvPtr env) {
    if (ast->type != ast::NodeType::List) {
      return false;
    }

    const auto list = std::static_pointer_cast<ast::List>(ast)->items;
    if (list.size() < 1 || list[0]->type != ast::NodeType::Symbol) {
      return false;
    }

    const auto symbol = std::static_pointer_cast<ast::Symbol>(list[0])->value;
    const auto defEnv = env->find(symbol);
    if (defEnv == nullptr) {
      return false;
    }
    const auto value = defEnv->get(symbol);

    if (value->type != ast::NodeType::UserFunc) {
      return false;
    }

    const auto func = std::static_pointer_cast<ast::UserFunc>(value);

    return func->isMacro();
  }

  /// Expands a macro
  EvalResult macroexpand(ast::TypePtr ast, runtime::EnvPtr env) {
    while (isMacroCall(ast, env)) {
      // These are all safe to do without checking as isMacroCall has already done the type checking
      const auto list       = std::static_pointer_cast<ast::List>(ast)->items;
      const auto macroName  = std::static_pointer_cast<ast::Symbol>(list.front())->value;
      const auto macro      = std::static_pointer_cast<ast::UserFunc>(env->find(macroName)->get(macroName));

      const auto arguments = std::vector<ast::TypePtr>(list.begin() + 1, list.end());

      if (arguments.size() < macro->paramCount) {
        rtnErrorAt(
            ast,
            "Expected at least " + std::to_string(macro->paramCount) + " arguments got " + std::to_string(arguments.size()) + " for user defined macro"
        );
      }

      const auto macroEnv = runtime::Environment::create(env, ast->toDummyToken(), macro->params, arguments);
      const auto result   = eval(macro->ast, macroEnv);
  ...
  EvalResult eval(ast::TypePtr input, runtime::EnvPtr env) {
    while (true) {
      if (input->type == ast::NodeType::List) {
        // If it's a list, expand the macro
        const auto expandResult = macroexpand(input, env);

        if (!expandResult) {
          return expandResult;
        }
        input = expandResult.right;

        if (input->type != ast::NodeType::List) {
          return evalAST(input, env);
        }

        // Once expanded if it's still a list, try to eval it
        ...
        } else if (isSymbol(params.front(), "defmacro!")) {
          return defmacro(input, std::vector<ast::TypePtr>(params.begin() + 1, params.end()), env);

        } else if (isSymbol(params.front(), "macroexpand")) {
          const auto arguments = std::vector<ast::TypePtr>(params.begin() + 1, params.end());
          if (arguments.size() != 1) {
            rtnErrorAt(input, "macroexpand expects 1 argument, got " + std::to_string(arguments.size()));
          }
          return macroexpand(arguments.front(), env);

        } else if (isSymbol(params.front(), "let*")) {
          const auto arguments = std::vector<ast::TypePtr>(params.begin() + 1, params.end() - 1);
          if (params.size() != 3) {
            rtnErrorAt(input, "let* expects 2 arguments, got " + std::to_string(params.size() - 1));
          }
...
int main(int argc, char *argv[]) {
  ...
  // Declare two macro control structures
  mal::rep("(defmacro! cond (fn* (& xs) (if (> (count xs) 0) (list 'if (first xs) (if (> (count xs) 1) (nth xs 1) (throw \"odd number of forms to cond\")) (cons 'cond (rest (rest xs)))))))", env);
  mal::rep("(defmacro! or (fn* (& xs) (if (empty? xs) nil (if (= 1 (count xs)) (first xs) `(let* (or_FIXME ~(first xs)) (if or_FIXME or_FIXME (or ~@(rest xs))))))))", env);
